...
<body>
	...
	<form role="form" action="/examens/{{$examen->id}}"  enctype="multipart/form-data" method="post">
		...
		<label>User ID</label>
		<input class="form-control" name="user_id" value="{{$examen->user_id}}" placeholder="User examen">
		<label>Consulation ID</label>
		<input class="form-control" name="consultation_id" value="{{$examen->consultation_id}}" placeholder="Consultation ID">
		<label>Nom</label>
